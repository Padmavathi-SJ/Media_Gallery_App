...
const Mom = () => {
  ...
  // Fetch Mom images from the backend
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/mom/get-images');
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/mom/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="mom-container relative mt-0">
