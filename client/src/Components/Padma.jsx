...
const Padma = () => {
  ...
  // Fetch Padma images from the backend
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/padma/get-images');
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/padma/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="padma-container relative mt-0">
