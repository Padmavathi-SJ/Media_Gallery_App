...
import Masonry from 'react-masonry-css'; // For masonry grid
import CadetsTopBar from './CadetsTopBar'; // Import the CadetsTopBar component
import './style.css';
import { FiUpload } from 'react-icons/fi'; // Icon for upload modal
...
const Akila = () => {
  ...
  // Fetch Akila images from the backend
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/akila/get-images');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setImages((prevImages) => [...prevImages, newImage]); // Update the images
  ...
  return (
    <div className="akila-container relative mt-0"> {/* Set relative positioning for the container */}
      {/* CadetsTopBar Component */}
      <CadetsTopBar />

      {/* Upload Icon aligned at the top-right corner */}
      <div className="upload-icon-container absolute mr-40  mt-4 top-2 right-2"> {/* Position it at top-right */}
        <button
      ...
      {showUploadModal && (
        ...
        >
          ...
          >
            ...
            />
            {uploadStatus && (
              <p className={`text-center ${uploadStatus === 'Image uploaded successfully!' ? 'text-green-600' : 'text-red-600'} mb-3`}>
                {uploadStatus}
              </p>
            )}
...
>
  {images.map((image, index) => (
    <img
      key={index}
      src={`http://localhost:5000${image.image_url}`} // Ensure correct URL path
      alt={`Akila Image ${index + 1}`} // Add descriptive alt text
